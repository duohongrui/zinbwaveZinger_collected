#' @export
reEstimateExprFraction4 <- function(counts,
                                    offset,
                                    lambda,
                                    phi){
...
#' @export
reEstimatePhiMoM4 <- function(counts,
                              lambda,
                              offset,
                              phi){
...
#' @export
getPhiMoMPositive4 <- function(counts,
                               lambda,
                               offset){
  #countsModel = counts[counts>0]
  #offsetModel = offset[counts>0]
...
#' @export
getDatasetMoMPositive <- function(counts,
                                  drop.extreme.dispersion = FALSE,
                                  cpm = "AveLogCPM",
                                  MoMIter = 10){
  ...
  ## estimate
  lambdaMoM <- apply(dFiltered$counts, 1, function(x) getExprFraction4(counts = x,
                                                                       offset = colSums(dFiltered$counts)))
  dispMoM <- vector(length = nrow(dFiltered$counts))
  for(i in 1:nrow(dFiltered$counts)){
    dispMoM[i] <- getPhiMoMPositive4(counts = dFiltered$counts[i,],
                                     offset = colSums(dFiltered$counts),
                                     lambda = lambdaMoM[i])
  }
  ...
  for(j in 1:MoMIter){
    ...
    for(i in 1:nrow(dFiltered$counts)){
      lambdaMoM[i] <- reEstimateExprFraction4(counts = dFiltered$counts[i,],
                                              offset = colSums(dFiltered$counts),
                                              phi = dispMoM[i],
                                              lambda = lambdaMoM[i])
    }
    for(i in 1:nrow(dFiltered$counts)){
      dispMoM[i] <- reEstimatePhiMoM4(counts = dFiltered$counts[i,],
                                      offset = colSums(dFiltered$counts),
                                      lambda = lambdaMoM[i],
                                      phi = dispMoM[i])
    }
  ...
  avCpm <- edgeR::aveLogCPM(d, normalized.lib.sizes = FALSE)
  ...
  dFiltered$AveLogCPM <- edgeR::aveLogCPM(dFiltered, normalized.lib.sizes = FALSE)
  ...
  params <- data.frame(dispersion = params[,1],
                       lambda = params[,2],
                       aveLogCpm = dFiltered$AveLogCPM,
                       propZeroGene = propZeroGene)
  ...
  list(dataset.AveLogCPM = dataset.AveLogCPM,
       dataset.dispersion = dataset.dispersion,
       dataset.lib.size = dataset.lib.size,
       dataset.nTags = dataset.nTags,
       dataset.propZeroFit = zeroFit,
       dataset.lambda = lambda,
       dataset.propZeroGene = propZeroGene,
...
NBsimSingleCell_zinbwaveZinger <- function(
  ...
  sim.fun <- function(object){
    ...
    zeroProbMatLink <- matrix(predict(zeroFit,
                                      newdata = data.frame(logLibHlp = libPredict,
                                                           midsHlp = cpmPredict),
                                      type="link"),
                              byrow = FALSE,
                              ncol = nlibs,
    ...
    ## simulate negative binomial counts
    mu <- sweep(Lambda, 2, lib.size, "*")
    zeroProbNegBin <- matrix(dnbinom(0, mu = mu, size = 1/Dispersion),
                             nrow = nTags,
                             ncol = nlibs,
                             byrow = FALSE)
    expectedZeroProbablityNegBinomial <- rowMeans(zeroProbNegBin)
    ...
    mu[mu<0.1] <- 0.1
    counts <- matrix(rnbinom(n = nTags*nlibs,
                             mu = mu,
                             size = 1/Dispersion),
                     nrow = nTags,
                     ncol = nlibs,
    ...
    dropoutMatrix <- 1-matrix(rbinom(n = nTags*nlibs,
                                     size = 1,
                                     prob = zeroProbMat),
                              nrow = nTags,
                              ncol = nlibs,
    ...
    while(length(allDropoutId) > 0){
      dropoutMatrix[allDropoutId,] <- 1-matrix(rbinom(n = length(allDropoutId)*nlibs,
                                                      size = 1,
                                                      prob = zeroProbMat[allDropoutId,]),
                                               nrow = length(allDropoutId),
                                               ncol = nlibs,
                                               byrow = FALSE)
      allDropoutId <- which(rowSums(dropoutMatrix) == 0)
    }
    ...
    while(length(zeroCountsId)>0 ){
      counts[zeroCountsId,] <- matrix(rnbinom(n = length(zeroCountsId)*nlibs,
                                              mu = mu[zeroCountsId,],
                                              size=1/Dispersion[zeroCountsId,]),
                                      nrow = length(zeroCountsId),
                                      ncol = nlibs,
                                      byrow = FALSE)
  ...
  if(is.null(params)){
    dataset <- getDatasetZTNB(counts = dataset,
